...
The game still ships as a single HTML file (`index.html`), but since the
Phase 1 enhancement work `index.html` is **auto-generated** from
`src/game.html` by `build.mjs`. Edit `src/game.html`, then rebuild.

```bash
git clone https://github.com/khaaliswooden-max/1977.git
cd 1977
node build.mjs            # regenerate ./index.html from ./src/game.html
...
- Keep the shipped `index.html` artifact a single file
- Edit `src/game.html`, not `index.html` — `index.html` is regenerated by `build.mjs`
